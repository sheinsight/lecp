...
describe("bundless dts(tsgo) alias ok", async () => {
	beforeAll(async () => {
	...
	it("bundless esm dts ok", async () => {
		...
		expect(fileMap["index.d.ts"]).not.toContain(`export { a } from "@/util";`);
	...
	it("bundless cjs dts ok(.cjs)", async () => {
		...
		expect(fileMap["index.d.cts"]).not.toContain(`export { a } from "@/util";`);
